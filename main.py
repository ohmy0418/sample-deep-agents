...
# psycopg 키워드 형식 접속 문자열. 비밀번호에 특수문자가 있어도 안전한 형식.
conn_string = (
...
with PostgresStore.from_conn_string(conn_string) as store:
    store.setup()
    print(f"[setup] PostgreSQL store 준비 완료 ")
    ...
    # ── [확인 1] DB(store)에 실제로 어떤 key 로 저장됐는지 ──
    print("── store 저장 확인 ──")
    for item in store.search(SKILLS_NAMESPACE):
        print(f"  저장된 key: {item.key}")
 
    # ── [확인 2] Agent 실행 전, /skills/ 아래에서 skill 이 보이는지 ──
    ls_result = backend.ls("/skills/")
    entries = ls_result["entries"] if isinstance(ls_result, dict) else ls_result.entries
    found = [e["path"] if isinstance(e, dict) else e.path for e in entries]
    print(f"── /skills/ 아래 발견된 항목: {found}")
    print(f"   (3개면 skill 로드 성공)\n")
